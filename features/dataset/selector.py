...
class SampleSelector:
    """
    Helper focused on selecting, filtering, and splitting a FeaturesDataset.

    This class manipulates domain-level feature families, such as:
    - entropy
    - variance
    - cn_alpha
    - subject_age
    ...
    def n_subjects(self) -> int:
        """Return the number of subject-level datasets."""
        return len(self.dataset.participant_datasets)

    @property
    def n_features(self) -> int:
        """Return the number of explanatory features."""
        return self.X().shape[1]

    def X(self):
        """Return the explanatory matrix of the current dataset."""
        return self.dataset.X

    @staticmethod
    def _ensure_non_empty_list(values: Iterable[str], *, name: str) -> list[str]:
        """Convert an iterable to a non-empty list."""
        result = list(values)
    ...
    def _connectivity_feature_to_band_name(cls, feature_name: str) -> str:
        """Convert a connectivity feature name to its band name."""
    ...
    def _split_requested_features(
        ...
        """Split requested features into scalar, connectivity, and subject features."""
    ...
    def _validate_requested_features(
        ...
        """Validate that requested features exist in the current dataset."""
    ...
    def _build_same_dataset_type(
        ...
        """
        Rebuild a dataset with the same type as ``self.dataset``.

        Important
        ---------
        If the current dataset is restricted, selected features are preserved.
        """
        from .base import FeaturesDataset
        from .selected import SelectedFeaturesDataset
    ...
    def select(self, features: Sequence[str]) -> "SelectedFeaturesDataset":
        """Select a restricted dataset from domain-level feature names."""
        from .base import FeaturesDataset
        from .participant import SingleParticipantProcessedFeatureDataset
    ...
    def select_features(self, features: Sequence[str]) -> "SampleSelector":
        """Return a new selector restricted to selected features."""
        return self._clone_with_dataset(self.select(features))

    def drop(self, features: Sequence[str]) -> "SelectedFeaturesDataset":
        """Return a restricted dataset after dropping selected feature families."""
    ...
    def drop_features(self, features: Sequence[str]) -> "SampleSelector":
        """Return a new selector after dropping selected feature families."""
        return self._clone_with_dataset(self.drop(features))

    def select_subject_ids(self, subject_ids):
        """Return a dataset restricted to selected subject identifiers."""
    ...
    def select_subject_ids_selector(self, subject_ids):
        """Return a new selector restricted to selected subject identifiers."""
        return self._clone_with_dataset(self.select_subject_ids(subject_ids))

    def filter_by_healthstate(self, healthstates):
        """Return a dataset restricted to selected health states."""
        parsed = [
    ...
    def filter_by_healthstate_selector(self, healthstates):
        """Return a new selector restricted to selected health states."""
    ...
    def group_train_test_split(
        ...
        """Split the dataset into train and test sets by subject identifier."""
        subject_ids = list(
            dict.fromkeys(
    ...
    def group_train_val_test_split(
        ...
        """Split the dataset into train, validation, and test sets by subject ID."""
        subject_ids = list(
            dict.fromkeys(
                p.subject.id for p in self.dataset.participant_datasets
            )
